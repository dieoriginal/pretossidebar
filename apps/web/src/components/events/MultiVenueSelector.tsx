...
import { getAllVenues, Venue, initializeDefaultVenues } from "@/lib/venuesDb";
import { DEFAULT_VENUES } from "@/lib/default-venues";
...
export function MultiVenueSelector({
  ...
  const [selectedCity, setSelectedCity] = useState(city || "__all__");

  useEffect(() => {
    loadVenues();
  }, []);

  // Inicializar com prop city apenas na montagem
  useEffect(() => {
    if (city !== undefined) {
      setSelectedCity(city || "__all__");
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []); // Apenas na montagem

  // Notificar mudanças quando o usuário muda internamente
  const handleCityChange = (newCity: string) => {
    setSelectedCity(newCity);
    if (onCityChange) {
      onCityChange(newCity === "__all__" ? "" : newCity);
    }
  };

  const loadVenues = async () => {
    try {
      // Initialize with default venues if database is empty
      if (DEFAULT_VENUES && DEFAULT_VENUES.length > 0) {
        await initializeDefaultVenues(DEFAULT_VENUES);
      }
      const venues = await getAllVenues();
      setAllVenues(venues);
    } catch (error) {
      console.error("Error loading venues:", error);
      // Fallback: try to load without initialization
      try {
        const venues = await getAllVenues();
        setAllVenues(venues);
      } catch (e) {
  ...
  // Filtrar venues por lotação e cidade
  const filteredVenues = useMemo(() => {
    return allVenues.filter(venue => {
      ...
      // Filtro por cidade
      const matchesCity = selectedCity === "__all__" || venue.city === selectedCity;
  ...
  return (
    <div className="space-y-4">
      {/* Capacity Input */}
      <div>
        <Label htmlFor="requiredCapacity">Lotação Requerida</Label>
        <Input
          id="requiredCapacity"
          type="number"
        ...
        <div className="text-xs text-slate-500 dark:text-slate-400 mt-1">
          Apenas venues com lotação ≥ {requiredCapacity} serão mostradas
        </div>
      </div>
        ...
        <CardContent className="space-y-4">
          ...
          {filteredVenues.length === 0 ? (
            <div className="text-center py-8 text-slate-500">
              Nenhuma venue encontrada com lotação ≥ {requiredCapacity}
              {selectedCity !== "__all__" && ` em ${selectedCity}`}
            </div>
          ) : (
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-slate-600 dark:text-slate-400 mt-1">
                          ...
                          <div className="flex items-center gap-1 mt-1">
                            <Users className="h-3 w-3" />
                            Lotação: {venue.capacity} pessoas
                          </div>
                          {venue.contactEmail && (
