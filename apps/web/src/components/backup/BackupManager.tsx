...
function BackupManagerFallback() {
  return (
    <Card>
      <CardHeader>
        <CardTitle>Gestão de Backups</CardTitle>
        <CardDescription>
          Para usar backups com utilizador, configura Clerk (<code>NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY</code> e{" "}
          <code>CLERK_SECRET_KEY</code>).
        </CardDescription>
      </CardHeader>
      <CardContent>
        <p className="text-sm text-muted-foreground">
          Clerk não está configurado neste ambiente, então os backups por utilizador estão indisponíveis.
        </p>
      </CardContent>
    </Card>
  );
...
function BackupManagerWithClerk() {
  const { user } = useUser();
...
export function BackupManager() {
  ...
  if (!isClerkConfigured) return <BackupManagerFallback />;
  return <BackupManagerWithClerk />;
}
