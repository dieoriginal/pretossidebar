...
export default function MiniTourPlannerPage() {
  ...
  const [useRealMap, setUseRealMap] = useState(true) // Ativar Mapbox por padrão
  ...
  const MapboxMap = useMemo(() => dynamic(() => import("./MapboxMap"), { ssr: false }), [])
  ...
  return (
    <div className="h-[calc(100vh-64px)] w-full flex flex-col">
      ...
      <div className="flex-1 grid grid-cols-1 md:grid-cols-[1fr_360px]">
        {/* Map */}
        <div ref={mapRef} className="relative overflow-hidden h-full w-full">
          {useRealMap ? (
            <>
              <MapboxMap
                stops={project.stops}
