          ...
          <div className="sticky-top">
            <div className="flex gap-2">
              ...
              <Button variant="outline" onClick={()=>{
                const trackTitles = [
                  "Fidúcia", "Pleurotus", "Eudaimonia", "biópsia", "VSOP Superior", "Divergente",
                  "episteme", "Vixerunt", "morbidez", "paideia", "elite Mouseion", "saevitia",
                  "Doxa", "Denilson Africanus'", "Craftsmen", "Courvoisier", "Legião",
                  "principium individuationis", "DOMUS AUREA (PALÁCIO)", "busca pela eudaimonia",
                  "G'Vine Floraison", "Epimanes (The Mad)", "idempotente", "SULPICIA",
                  "Segundo Triunvirato", "imhotep", "Johann Wolfgang von Goethe", "Hurstwic",
                  "Saúde Infantil", "Urpflanze", "Imhotep", "RIP julius Cesar", "altar",
                  "Rosetta Stone (Pedra Rosetta)", "ataraxia", "devir contínuo", "Numidia",
                  "Lepsius", "eqüipolência", "Tartuffe", 'Organic evolution"eqüipolência"',
                  "odium", "finitude", "Die Freien", "cd back-offices/coo && npm run build",
                  "rapsódia-----", "Escrutínio", "Diplomacia", "Nile Delta", "Hegemonia",
                  "Antiópia", "Libertopia", "ecletismo", "Parricídio", "Secrecy", "Matricídio",
                  "Calpernia Benza"
                ];
                const existingTitles = new Set(proj.tracks.map(t => t.title));
                const newTracks = trackTitles
                  .filter(title => !existingTitles.has(title.trim()))
                  .map(title => ({ id: uid('trk'), title: title.trim(), artist: proj.artist || undefined }));
                if (newTracks.length > 0) {
                  savePatch({ tracks: [...proj.tracks, ...newTracks] });
                  alert(`✅ Adicionadas ${newTracks.length} novas faixas!`);
                } else {
                  alert("Todos os títulos já existem no projeto.");
                }
              }}>Importar títulos</Button>
            </div>
          </div>
            ...
            <tbody>
              {filteredTasks.map(t => (
                <tr key={t.id}>
                  ...
                  <td>
                    <Select value={t.stepSlug||'__none__'} onValueChange={(v)=> savePatch({ tasks: proj.tasks.map(x=> x.id===t.id ? { ...x, stepSlug: v === '__none__' ? undefined : v } : x) })}>
